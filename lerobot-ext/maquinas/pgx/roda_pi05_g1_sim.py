...
Por que o comando vai por DDS, e não pela classe `UnitreeG1`:
  ...
  caminho move o robô foi medido com o `maquinas/pgx/testa_dds_sim.py`: senoide de ±0,8 rad comandada,
  1,588 rad de amplitude medida.

    python maquinas/pgx/roda_pi05_g1_sim.py --passos 3600 --tempo-real --v-web=8088
"""
import os
import sys
...
def carrega_env(cameras: list[str], porta: int, env_dir: str | None = None, na_tela: bool = False):
    ...
    `env_dir` é a saída do `maquinas/pgx/prepara_cena_caixa.sh`: a mesma árvore, com a cena das mesas e
    ...
    if not (raiz / "env.py").is_file():
        raise SystemExit(f"❌ {raiz} não tem env.py — rode antes: bash maquinas/pgx/prepara_cena_caixa.sh")
...
def main() -> int:
    ...
    p.add_argument("--env-dir", default=None,
                   help="pasta do ambiente (padrão: cache do HF). Use a cópia com as mesas e a "
                        "caixa azul feita por maquinas/pgx/prepara_cena_caixa.sh")
