...
IN_Y = SUB_Y + SUB_H
THINK_S, AFTER_S, INTRO_S, OUTRO_S = 3.0, 3.0, 4.0, 6.0   # AFTER_S: pausa no fim de cada passo
...
def mmss(sec):
    """Tempo no formato 00m00s."""
    t = int(round(float(sec or 0)))
...
class Story:
    ...
    def header(self, d, current):
        ...
        n = len(self.calls)   # episodio longo: quadros menores; acima de 30, numero so a cada 5
        pitch, box = (46, 38) if n <= 15 else (32, 26) if n <= 30 else (20, 16)
        ...
        for c in self.calls:
            ...
            f = FH if box == 38 else FXS if box == 26 else font(10)
            d.rounded_rectangle((x, 16, x + box, 48), 7, fill=fill, outline=INK if now else None, width=2)
            if box > 16 or c["call"] % 5 == 0 or now:
                d.text((x + box / 2 - d.textlength(str(c["call"]), font=f) / 2, 32 - f.size / 2 - 2), str(c["call"]), font=f,
                       fill=(12, 12, 16) if (done or now) else MUTED)
            x += pitch

    def sub_schedule(self, text, speech_frames):
        """Blocos de ate 2 linhas e o quadro (desde o inicio do passo) em que cada um entra, proporcional ao texto."""
        d = ImageDraw.Draw(Image.new("RGB", (1, 1)))
        lines = wrap(d, text or "", FSUB, PANEL_X - 80)
        blocks = [" ".join(lines[i:i + 2]) for i in range(0, len(lines), 2)] or [""]
        total = sum(len(b) for b in blocks) or 1
        start, out = int(0.3 * FPS), []
        for b in blocks:
            out.append((start, b))
            start += int(speech_frames * len(b) / total)
        return out

    @staticmethod
    def sub_at(schedule, j):
        cur = schedule[0][1]
        for f, b in schedule:
            if j >= f:
                cur = b
    ...
    def inputs(self, img, d, call):
        ...
        y0 = IN_Y + 38
        cache = self.__dict__.setdefault("_cam_cache", {})
        load = lambda f, w, hh: cache.setdefault((str(f), w, hh), fit(Image.open(f).convert("RGB"), w, hh))
        h = load(step_dir / "head_camera.png", 640, 360)
        ...
        for cam, lab in (("left_wrist_camera", "punho esq."), ("right_wrist_camera", "punho dir.")):
            im = load(step_dir / f"{cam}.png", 310, 176)
    ...
    def clocks(self, call, phase, progress):
        """(em acao, total). Em acao: tempo simulado do robo se movendo. Total: tempo real desde a 1a chamada
        (raciocinio do modelo + sondas + execucao), pelos carimbos de horario das chamadas."""
        import datetime as dt
        ts = lambda c, k: dt.datetime.fromisoformat(c[k]).timestamp() if c.get(k) else None
        t0 = ts(self.calls[0], "sent_at") or 0
        nxt = next((c for c in self.calls if c["call"] > call["call"]), None)
        sim0 = call.get("sim_time_s", 0)
        sim1 = nxt.get("sim_time_s", sim0) if nxt else self.report.get("sim_seconds", sim0)
        wall_rx = (ts(call, "received_at") or t0) - t0
        wall_next = ((ts(nxt, "sent_at") or 0) - t0) if nxt else wall_rx + (sim1 - sim0)
        if phase == "think":
            return sim0, wall_rx
        return sim0 + progress * (sim1 - sim0), wall_rx + progress * max(0.0, wall_next - wall_rx)
    ...
    def panel(self, d, call, phase, reveal=1.0, progress=0.0, sample=None):
        ...
        em_acao, total = self.clocks(call, phase, progress)
        d.text((x + 170, y + 2), "EM AÇÃO", font=FXS, fill=MUTED)
        d.text((x + 170, y + 16), mmss(em_acao), font=font(16, True, mono=True), fill=OK)
        d.text((x + 270, y + 2), "TOTAL (com raciocínio)", font=FXS, fill=MUTED)
        d.text((x + 270, y + 16), mmss(total), font=font(16, True, mono=True), fill=WARN)
        y += 38
        ...
        if call.get("latency_s"):
            d.text((cx + 2, y + 6), f"resposta em {mmss(call['latency_s'])}  ·  {(call.get('sent_at') or '')[11:19]} UTC",
                   font=FS, fill=MUTED)
        y += 44
        y = self.section(d, y, "TAREFA", self.task, color=INK, fnt=FS, max_lines=2)
        args = call.get("arguments", {})
        pt = ((self.narration or {}).get("panel_pt") or {}).get(str(call["call"])) or {}
        see = pt.get("ve") or call.get("what_i_see")
        why = pt.get("porque") or args.get("reason", call.get("error", ""))
        tag = " (tradução; original em inglês no log)" if pt.get("ve") else ""
        if see:
            y = self.section(d, y, "O QUE ELE VÊ" + tag, see, fnt=FS, max_lines=6, reveal=min(1, reveal * 1.6))
        y = self.section(d, y, "RACIOCÍNIO DECLARADO" + tag, why,
    ...
    def cost_box(self, d, call, reveal):
        ...
        d.text((x0 + 14, y0 + 128), f"espera    {mmss(lat):>7}", font=font(15, mono=True), fill=INK)
    ...
    # ---------------- render ----------------
    def _grid_before(self, call):
        """Ultimo quadro da grade antes deste passo (um trecho renderizado em paralelo comeca igual ao anterior)."""
        prev = [c for c in self.calls if c["call"] < call["call"] and c["_frames"]]
        return grid_of(prev[-1]["_frames"][-1] if prev else self.frames[SETTLE_FRAMES - 1])

    def _intro(self, wr, title):
        nar, case = self.narration or {}, self.meta["case"]
    ...
    def _outro(self, wr):
        ...
        ok, last, ab = r.get("accepted"), self.calls[-1], r.get("aborted") or {}
        ...
        wr.put(self.card("ACEITO" if ok else "REPROVADO", [
            f"Caneca levantada {r.get('lift_m', 0)*100:.1f} cm (mínimo 4,8) · retida 2 s: {'sim' if r.get('retained_for_2s') else 'não'} · "
            f"em pé: {'sim' if r.get('upright_grasp') else 'não'} (inclinação antes do fecho {r.get('cup_tilt_deg_before_close', 0):.0f}°, na janela final "
            f"{r.get('cup_tilt_deg_at_hold') or 0:.1f}°, limite 15°){why}",
            f"{len(self.calls)} chamadas ao modelo · {r.get('rejected_calls', 0)} rejeitadas · {mmss(r.get('sim_seconds'))} simulados · "
            f"{mmss(sum(c.get('latency_s') or 0 for c in self.calls))} esperando o modelo",
    ...
    def play(self, wr, title=None, calls=None, intro=True, outro=True):
        """Toca o episodio (ou um trecho de passos) no Writer: intro, steps, veredito."""
        put, nar = wr.put, self.narration or {}
        calls = self.calls if calls is None else calls
        if intro:
            self._intro(wr, title)
        last_grid = self._grid_before(calls[0])
        for call in calls:
            text = nar.get("steps", {}).get(str(call["call"]), "")
            wr.last_speech_frames = 0
            need = wr.speak(text)
            sched = self.sub_schedule(text, wr.last_speech_frames)
            sub = lambda j: self.sub_at(sched, j)
            frames = call["_frames"]
            move_n = len(frames)                       # a fala cobre "decidindo" + movimento
            think_n = max(int(THINK_S * FPS), need - move_n)
            act_n = move_n + int(AFTER_S * FPS)       # e SO depois vem a pausa de fim de passo
            reveal_n = int(THINK_S * FPS * 0.8)
            i = 0
            while i < think_n:
                im = self.frame(last_grid, call, "think", sub(i), reveal=min(1.0, (i + 1) / reveal_n))
                if i + 1 >= reveal_n:   # painel completo: repete o quadro ate a legenda trocar de bloco
                    nxt = min([f for f, _ in sched if f > i] + [think_n])
                    put(im, nxt - i); i = nxt
                else:
                    put(im); i += 1
            j0 = think_n
            if not frames:
                i = 0
                while i < act_n:
                    im = self.frame(last_grid, call, "act", sub(j0 + i), progress=1.0)
                    nxt = min([f - j0 for f, _ in sched if f - j0 > i] + [act_n])
                    put(im, nxt - i); i = nxt
                continue
            sample = None
            for k, fp in enumerate(frames):
                ...
                put(self.frame(last_grid, call, "act", sub(j0 + k), progress=(k + 1) / len(frames), sample=sample))
            put(self.frame(last_grid, call, "act", sub(j0 + len(frames)), progress=1.0, sample=sample), int(AFTER_S * FPS))
        if outro:
            self._outro(wr)

    def render(self, out, workers=1):
        voice = (self.narration or {}).get("voice", "pt-BR-AntonioNeural")
        if workers <= 1:
            wr = Writer(out, voice); self.play(wr); wr.close(); return
        # trechos de passos em paralelo (um processo por nucleo), emendados sem recodificar o video
        import multiprocessing as mp
        idx = [list(x) for x in np.array_split(np.arange(len(self.calls)), min(workers, len(self.calls))) if len(x)]
        jobs = [(str(self.ep), str(out.with_suffix(f".part{i:02d}.mp4")), ix[0], ix[-1], i == 0, i == len(idx) - 1, voice)
                for i, ix in enumerate(idx)]
        with mp.get_context("spawn").Pool(len(jobs)) as pool:
            parts = pool.map(_render_part, jobs)
        lst = out.with_suffix(".parts.txt")
        lst.write_text("".join(f"file '{Path(v).resolve()}'\n" for v, _ in parts))
        silent = out.with_suffix(".silent.mp4")
        subprocess.run(["ffmpeg", "-y", "-loglevel", "error", "-f", "concat", "-safe", "0", "-i", str(lst),
                        "-c", "copy", str(silent)], check=True)
        audio = np.concatenate([np.frombuffer(wave.open(w).readframes(10 ** 9), np.int16) for _, w in parts])
        ...
        for v, w in parts:
            Path(v).unlink(); Path(w).unlink()
        for f in (lst, silent, wav):
            f.unlink()


def _render_part(job):
    ep, out, i0, i1, intro, outro, voice = job
    st = Story(Path(ep))
    wr = Writer(out, voice)
    st.play(wr, calls=st.calls[i0:i1 + 1], intro=intro, outro=outro)
    return wr.close(mux=False)
...
class Writer:
    ...
    def speak(self, text):
        ...
        self.last_speech_frames = int(len(pcm) / SR * FPS)
        return self.last_speech_frames + int(0.6 * FPS)

    def close(self, mux=True):
        self.ff.stdin.close(); self.ff.wait()
        if not self.audio and mux:
            self.silent.rename(self.out); return
        exact = self.cursor * SR // FPS          # trecho emendado: audio com a duracao exata do video
        total = np.zeros(exact if not mux else exact + SR, np.int32)
        ...
        if not mux:
            keep = self.out.with_suffix(".wav")
            wav.replace(keep)
            return str(self.silent), str(keep)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out")
    ap.add_argument("--workers", type=int, default=1, help="processos em paralelo (trechos de passos)")
    a = ap.parse_args()
    ep = Path(a.episode_dir)
    out = Path(a.out) if a.out else ep / "direct-story.mp4"
    Story(ep).render(out, workers=a.workers)
    print(out)
