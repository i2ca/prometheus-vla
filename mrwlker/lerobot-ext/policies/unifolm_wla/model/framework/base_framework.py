...
from unifolm_wla.model.tools import FRAMEWORK_REGISTRY, FrameworkTools, auto_get_trainable_modules
        ...
        if is_pkg:
            ...
            for _, sub_name, _ in pkgutil.iter_modules([str(sub_dir)]):
                if sub_name.startswith("_"):
                    continue
                importlib.import_module(f"unifolm_wla.model.framework.{module_name}.{sub_name}")
        else:
            importlib.import_module(f"unifolm_wla.model.framework.{module_name}")
