...
class TrainerUtils:
    ...
    @staticmethod
    def load_pretrained_backbones(model, checkpoint_path=None, reload_modules=None, remove_modules=None):
        """
        load checkpoint:
        ...
        if remove_modules:
            for k in list(checkpoint.keys()):
                if any(module_name in k for module_name in remove_modules.split(",")):
                    checkpoint.pop(k)
                    print(f"⚠️ removed weights from checkpoint: {k}")
    ...
    def _reset_dataloader(dataloader, epoch_counter):
        ...
        # The multi_source pipeline passes a custom batch_sampler, so the epoch-aware
        # sampler is at dataloader.batch_sampler (dataloader.sampler is a default
        # SequentialSampler with no set_epoch). Try batch_sampler first, then sampler.
        for s in (getattr(dataloader, "batch_sampler", None), getattr(dataloader, "sampler", None)):
            if callable(getattr(s, "set_epoch", None)):
                s.set_epoch(epoch_counter)
                break
