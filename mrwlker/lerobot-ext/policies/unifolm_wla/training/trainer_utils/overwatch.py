...
def initialize_overwatch(name: str) -> Union[DistributedOverwatch, PureOverwatch]:
    # DataLoader spawn workers re-execute the training entrypoint via
    # multiprocessing's `runpy.run_path(__main__)`, which re-runs this module at
    # import time (tools.py builds the module-level `overwatch`). In a worker we
    # must NOT build a DistributedOverwatch: its `PartialState()` calls
    # `torch.cuda.set_device()` and binds a full ~520 MiB CUDA context per worker
    # (×num_workers×num_ranks of pure waste — the data path is CPU-only).
    #
    # Detecting the worker is subtle. At the moment this runs (module import during
    # the re-exec), BOTH of these are still the parent's values / not yet set:
    #   - sys.argv -> already overwritten with the PARENT's argv (no --multiprocessing-fork)
    #   - type(current_process()) -> still `_MainProcess` (the SpawnProcess type is
    #     installed only AFTER the re-import finishes)
    # The signals that ARE correct this early (verified in-container):
    #   - current_process().name == "SpawnProcess-N"  (the .name is set pre-import)
    #   - __main__ module run_name == "__mp_main__"
    # Key off the process .name prefix.
